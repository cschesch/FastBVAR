function [ir,Omeg] = iresponse_sign_hmoments(errors,Phi,Sigma,hor,signrestriction,hmoments,f,cont)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 'iresponse_sign_hmoments' computes the impulse response functions using sign
% restrictions on the endogenous variables and higher order moments on the
% structural shocks
% Reference:
...
while d==0 && tol < 30000
    ...
    if d0 ==1
        ...
        % check higher-moments restrictions
        d = checkmrestrictions(hmoments,v, f);
    end
    if d==1 % stop
%         Omeg  = Omega;
        Omeg  = fsign * Omega;
        y     = iresponse(Phi,Sigma,hor,Omeg);
        ir    = y;
    else
        tol = tol + 1;
    end
